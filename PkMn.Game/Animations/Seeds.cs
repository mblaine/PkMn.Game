...
namespace PkMn.Game.Animations
{
    public class Seeds : Animation
    {
        ...
        public Seeds(MonsterRenderer renderer, int delay, int param = 0, int startAt = 0, int stopEarly = int.MaxValue)
        ...
        {
            Texture2D sprite = SpriteManager.EffectSprites["seeds"];

            int animationFrame = frameCounter / delay;
            int offset = renderer.ScreenEdgeDirection > 0 ? animationFrame % 4 : (3 - animationFrame) % 4;
